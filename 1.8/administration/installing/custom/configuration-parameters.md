...
These configuration parameters are specified in [YAML][1] format in your config.yaml file. During custom DC/OS installations the configuration file is used to generate a customized DC/OS build. 
...
*  `enable_docker_gc: 'true'` Run the docker-gc scripts once every hour. This is the default value for [cloud](/docs/1.8/administration/installing/cloud/) template installations.
*  `enable_docker_gc: 'false'` Do not run the docker-gc scripts once every hour. This is the default value for [custom](/docs/1.8/administration/installing/custom/) installations.
